...
export class ComposableStableExit implements BaseExit {
    public async query(
        ...
        const amountsWithoutBpt = {
            ...amounts,
            minAmountsOut: [...amounts.minAmountsOut.slice(0, bptIndex), ...amounts.minAmountsOut.slice(bptIndex+1)]
        };
    ...
    private getAmountsQuery(tokens: Token[], 
                            input: ExitInput,
                            bptIndex?:number): AmountsExit {
        switch (input.kind) {
    ...
    public buildCall(input: ExitCallInput): ExitBuildOutput {
        ...
        const amountsWithoutBpt = {
            ...amounts,
            minAmountsOut: [...amounts.minAmountsOut.slice(0, input.bptIndex), ...amounts.minAmountsOut.slice(input.bptIndex+1)]
        };
    ...
    private encodeUserData(kind: ExitKind, amounts: AmountsExit): Address {
        switch (kind) {
            ...
            case ExitKind.PROPORTIONAL:
                return ComposableStableEncoder.exitProportional(amounts.maxBptAmountIn);
            default:
                throw Error('Unsupported Exit Type');
        }
    }
