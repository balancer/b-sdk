...
export class ComposableStableExit implements BaseExit {
  public async query(
    ...
    const queryResult = await doQueryExit(input.rpcUrl, input.chainId, args);
      ...
      case ExitKind.UNBALANCED:
        return {
          minAmountsOut: tokens.map(
            (t) =>
              input.amountsOut.find((a) => a.token.isEqual(t))?.amount ?? 0n
          ),
  ...
  private encodeUserData(kind: ExitKind, amounts: AmountsExit): Address {
    switch (kind) {
      ...
      case ExitKind.SINGLE_ASSET:
        if (amounts.tokenOutIndex === undefined) throw Error("No Index");
